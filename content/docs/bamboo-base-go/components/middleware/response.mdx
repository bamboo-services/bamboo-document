...
func GetUser(ctx *gin.Context) {
    ...
    if err != nil {
        ...
        ctx.Error(xError.NewError(ctx, xError.NotFound, "用户不存在", true))
        return
    }
    xResult.Success(ctx, user)
}
...
// [!code highlight:2]
// 创建带错误码的错误
err := xError.NewError(ctx, xError.ParameterError, "参数格式错误", true)
ctx.Error(err)
```
...
func GetUser(ctx *gin.Context) {
    ...
    // 参数校验
    if id == "" {
        ctx.Error(xError.NewError(ctx, xError.ParameterError, "用户ID不能为空", true))
        return
    }
...
<Cards>
  <Card title="CORS 跨域" href="./cors" />
  <Card title="错误处理" href="../error" />
  <Card title="统一响应" href="../../core/result" />
</Cards>
