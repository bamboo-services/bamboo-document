...
nodeList.Append(xConsts.DatabaseKey, initDatabase())
nodeList.Append(xConsts.RedisClientKey, initRedis())

reg := xReg.Register(ctx, []xRegNode.RegNodeList{
    {Key: xConsts.RegNodeKey, Node: func(ctx context.Context) (any, error) {
        return nodeList, nil
    }},
})
```
