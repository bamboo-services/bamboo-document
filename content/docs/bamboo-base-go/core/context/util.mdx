...
```go
emailClient, err := xCtxUtil.GetEmailClient(ctx)
if err != nil {
    return nil, err
}
...
<TypeTable
  type={{
    ...
    MustGetEmailClient: {
      description: '获取邮件客户端（panic）',
      type: '(ctx context.Context) *xEmail.EmailClient',
      required: true,
    },
    GetEmailClient: {
      description: '获取邮件客户端（错误返回）',
      type: '(ctx context.Context) (*xEmail.EmailClient, *xError.Error)',
      required: true,
    },
  }}
/>
