...
```go
// 一元拦截器
import xGrpcIUnary "github.com/bamboo-services/bamboo-base-go/grpc/interceptor/unary"
...
- 将请求 ID 写入 `context`（`xConsts.RequestKey`）。
- 记录请求开始时间（`xConsts.UserStartTimeKey`）。
...
- 从主上下文提取 `xConsts.RegNodeKey`（节点初始化结果集合）。
...
```go
import xGrpcMiddle "github.com/bamboo-services/bamboo-base-go/grpc/middleware"
```
