...
flowchart TD
    ...
    C --> D[SnowflakeInit<br/>雪花算法节点]
    D --> E{opts 含 WithDatabase?}
    E -- 是 --> F[DatabaseInit<br/>GORM 建连 + AutoMigrate]
    E -- 否 --> G{opts 含 WithCache?}
    F --> G
    G -- 是 --> H[CacheInit<br/>Manager + Redis/Memory]
    G -- 否 --> I[业务自定义节点]
    H --> I
    I --> J[Init.Exec<br/>一次性执行全部装配]
    J --> K[engineInit<br/>创建 Gin 引擎]
    K --> L[路由注册<br/>按 opts.Routes 顺序挂载]
    L --> M[返回 *Reg]"
/>
...
| `gorm.io/driver/mysql` | v1.6+ | MySQL GORM 驱动 |
| `gorm.io/driver/postgres` | v1.6+ | PostgreSQL GORM 驱动 |
| `gorm.io/driver/sqlserver` | v1.6+ | SQL Server GORM 驱动 |
| `oracle-samples/gorm-oracle` | v1.1+ | Oracle GORM 驱动（依赖 godror + Instant Client） |
