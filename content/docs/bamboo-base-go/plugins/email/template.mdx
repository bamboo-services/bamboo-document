...
_base.html              ← 基础布局（页头、页脚、样式）
  └── {{template "content" .}}
        ├── verification.html   ← 定义 "content" 块：验证码
        ├── welcome.html        ← 定义 "content" 块：欢迎
        └── reset_password.html ← 定义 "content" 块：重置密码
```

- `_base.html` 提供统一的邮件外观（品牌标题、页脚声明、背景样式）
- 每个具体模板通过 `{{define "content"}}` 定义内容块
- 渲染时自动套用基础布局
...
- **文件格式**: 外部模板必须是 `.html` 后缀的文件，且使用 `{{define "content"}}` 定义内容块。
