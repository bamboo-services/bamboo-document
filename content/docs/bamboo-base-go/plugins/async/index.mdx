...
- **上下文分离**: 从请求上下文中提取组件引用（包括 RequestKey 链路追踪），注入到独立的 goroutine 上下文中，父上下文取消不影响异步任务
...
- **选项配置**: 支持 `WithName`、`WithDebug`、`WithLogger` 三种选项
- **链路追踪**: 自动继承父上下文的 RequestKey，支持全链路日志追踪
- **轻量级**: 纯工具库，无需注册到 `xMain.Runner`

## 模块导入

```go
import xAsync "github.com/bamboo-services/bamboo-base-go/plugins/async"
```
...
| 选项 | 类型 | 说明 |
|------|------|------|
| `WithName(name)` | `Option` | 设置异步任务名称，名称会显示在日志中 |
| `WithDebug()` | `Option` | 启用调试日志（输出开始执行、执行完成） |
| `WithLogger(logger)` | `Option` | 设置自定义日志器 |
...
3. 将 `ContextNodeList` 和 `RequestKey`（链路追踪 ID）注入新上下文中
...
parentCtx（请求上下文）
  ...
  └── UserStartTimeKey → time.Now()
          │
          │  Async() 提取 RegNodeKey + RequestKey
          ▼
asyncCtx（独立上下文）
  ├── RegNodeKey → ContextNodeList [DB, Redis, Snowflake, ...]  ← 组件引用
  ├── RequestKey → "uuid-xxx"                                    ← 链路追踪
  └── context.WithCancel → 支持 Cancel() 调用
```

异步任务中**不复制**请求级数据（`UserStartTimeKey` 等），因为异步任务是独立的执行单元。
