...
public class SystemInit extends InitSystemStartupHandler {
    ...
    public void init() {
        ...
        // 1. 创建算法处理器实例
        prepare = new InitAlgorithm(jdbcTemplate, tableDAO, migrateHandlerDAO,
                transactionTemplate, permissionDAO, roleDAO, systemDAO);
...
public class InitAlgorithm extends InitPrepareAlgorithmHandler {
    ...
    public InitAlgorithm(JdbcTemplate jdbcTemplate, TableHandlerDAO tableDAO,
        ...
        super(jdbcTemplate, tableDAO, migrateHandlerDAO, transactionTemplate);
...
```sql
CREATE TABLE IF NOT EXISTS `t_migrate`
(
