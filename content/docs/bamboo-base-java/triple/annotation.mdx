...
<TypeTable
  type={{
    message: {
      description: '校验失败时的错误提示信息。为空时使用默认格式。将作为 BusinessException 的消息内容。',
      type: 'String',
      default: '""（空字符串，使用默认消息）',
    },
    errorCode: {
      description: '校验失败时的错误码枚举。将作为 BusinessException 的错误码，便于调用方识别具体的校验错误类型。',
      type: 'ErrorCode',
      default: 'ErrorCode.PARAMETER_INVALID',
    },
  }}
/>
